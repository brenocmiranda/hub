jQuery( function( $ ){
	...
	$( '.submit-btn' ).prop( 'disabled', true ).on( 'click', function( e ){
		...
		let $submit = $(this),
			...
			com = $form.find('[name="com"]:checked').length > 0 ? $form.find('[name="com"]:checked').val().trim() : "",
	...
	*/
    ...
    if( window.useColorboxVideo ) { 
        setTimeout(() => {
			if( !$().colorbox ){
				hub.helpers.loadscript( 'https://hub.komuh.com/js/external/colorbox.min.js', { defer: true }, function(){
    ...
    if( window.galleryJson ) {
		setTimeout(() => {
			...
			if( !$().slick && !!$().colorbox ){
				hub.helpers.loadscript( 'https://hub.komuh.com/js/external/slick.min.js', { defer: true }, function(){
