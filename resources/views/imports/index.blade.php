                        ...
                        var data = [
                            @foreach($imports as $import)
                                { 
                                    ...
                                    'status': '{!! ( $report->status === "Na fila" ? '<div class="badge text-bg-secondary">' . $report->status . '</div>' : ( $report->status === "Executando" ? '<div class="badge text-bg-primary">' . $report->status . '</div>' : ( $report->status === "Arquivo sendo gerado" ? '<div class="badge text-bg-dark">' . $report->status . '</div>' : ( $report->status === "Pronto" ? '<div class="badge text-bg-success">' . $report->status . '</div>' : $report->status )))) !!}', 
